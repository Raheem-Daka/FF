...
const Settings = () => {
  ...
  return (
    <div className="flex min-h-screen text-sm overflow-x-hidden">
      <ProfileSidePanel />
      <div
        className="gap-4 lg:flex px-5 transition-all duration-300"
      >
        ...
        <div className="relative bg-white w-full h-[640px] lg:overflow-y-auto p-6 mt-6 lg:mt-27 rounded border border-gray-300">
          <h2 className="text-xl font-semibold text-gray-500">Active Sessions</h2>

          <ul className="mt-4 space-y-3 lg:overflow-y-auto lg:max-h-[500px]">
            {sessions.map((s) => (
              <li key={s.id} className="flex justify-between items-center border border-gray-300 p-3 rounded">
                <div className="">
                  <p className="text-sm font-medium">{s.device}</p>
                  <p className="text-xs text-gray-500">
                    {s.location} • {new Date(s.last_active).toLocaleString()}
                  </p>
                </div>

                <button
                  onClick={() => logoutSession(s.id)}
                  className="bg-red-500 p-2 rounded text-white text-xs lg:max-w-20 hover:bg-red-600 transition cursor-pointer"
                >
                  Logout
                </button>
              </li>
          ...
          <button
            ...
            className="mt-4 w-full bg-red-500 text-white py-2 rounded hover:bg-red-700 transition text-sm"
          >
